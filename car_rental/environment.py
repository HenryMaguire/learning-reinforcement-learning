            ...
            for r2 in range(max_rental_2 + 1):
                for ret1 in range(max_return_1 + 1):
                    for ret2 in range(max_return_2 + 1):
                        ...
                        p = (self._poisson_prob(self.rental_rates[0], r1) *
                             self._poisson_prob(self.rental_rates[1], r2) *
                             self._poisson_prob(self.return_rates[0], ret1) *
                             self._poisson_prob(self.return_rates[1], ret2))
                        
                        if p > 1e-8:  # Ignore very unlikely transitions
